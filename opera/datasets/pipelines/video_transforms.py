...
from mmdet.datasets.pipelines import MulRandomCrop as MMDetMulRandomCrop
...
class MulResize(MMDetResize):
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super(MulResize, self).__init__(*args, **kwargs)
    ...
    def __call__(self, results):
        ...
        results = super(MulResize, self).__call__(results)
        self._resize_keypoints(results)
        self._resize_areas(results)
        return results

    def __repr__(self):
        repr_str = super(MulResize, self).__repr__()[:-1] + ', '
        repr_str += f'keypoint_clip_border={self.keypoint_clip_border})'
        return repr_str

# 单样本多帧输入，数据集通用的数据增强
@PIPELINES.register_module()
class MulRandomFlip(MMDetRandomFlip):
    ...
    def __call__(self, results):
        ...
        results = super(MulRandomFlip, self).__call__(results)
        if results['flip']:
            # flip keypoints
            for key in results.get('keypoint_fields', []):
...
# 单样本多帧输入，数据集通用的数据增强
@PIPELINES.register_module()
class MulRandomCrop(MMDetMulRandomCrop):
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super(MulRandomCrop, self).__init__(*args, **kwargs)
    ...
    def __repr__(self):
        repr_str = super(MulRandomCrop, self).__repr__()[:-1] + ', '
        repr_str += f'kpt_clip_border={self.kpt_clip_border})'
        return repr_str

# 单样本3帧，数据集通用的数据增强
@PIPELINES.register_module()
class MulKeypointRandomAffineForFrames3:
    ...
    def __call__(self, results):
        ...
        prev_height = img_prev.shape[0] + self.border[0] * 2
        prev_width = img_prev.shape[1] + self.border[1] * 2
        
        img_now = results['img_now']
        now_height = img_now.shape[0] + self.border[0] * 2
        now_width = img_now.shape[1] + self.border[1] * 2
        
        img_next = results['img_next']
        next_height = img_next.shape[0] + self.border[0] * 2
        next_width = img_next.shape[1] + self.border[1] * 2

        # Center
        center_matrix = np.eye(3, dtype=np.float32)
        center_matrix[0, 2] = -img_now.shape[1] / 2  # x translation (pixels)
        center_matrix[1, 2] = -img_now.shape[0] / 2  # y translation (pixels)
        ...
        trans_x = random.uniform(0.5 - self.max_translate_ratio,
                                 0.5 + self.max_translate_ratio) * now_width
        trans_y = random.uniform(0.5 - self.max_translate_ratio,
                                 0.5 + self.max_translate_ratio) * now_height
        ...
        img_prev = cv2.warpPerspective(
            img_prev,
            warp_matrix,
            dsize=(prev_width, prev_height),
            borderValue=self.border_val)
        img_now = cv2.warpPerspective(
            img_now,
            warp_matrix,
            dsize=(now_width, now_height),
            borderValue=self.border_val)
        img_next = cv2.warpPerspective(
            img_next,
            warp_matrix,
            dsize=(next_width, next_height),
            borderValue=self.border_val)
        ...
        if num_bboxes:
            ...
            warp_bboxes[:, [0, 2]] = warp_bboxes[:, [0, 2]].clip(0, now_width)
            warp_bboxes[:, [1, 3]] = warp_bboxes[:, [1, 3]].clip(0, now_height)
...
# 单样本5帧，数据集通用的数据增强
@PIPELINES.register_module()
class MulKeypointRandomAffineForFrames5:
    """Random affine transform data augmentation.
    ...
    def __call__(self, results):
        ...
        prev_prev_height = img_prev_prev.shape[0] + self.border[0] * 2
        prev_prev_width = img_prev_prev.shape[1] + self.border[1] * 2
        ...
        next_next_height = img_next_next.shape[0] + self.border[0] * 2
        next_next_width = img_next_next.shape[1] + self.border[1] * 2
        ...
        img_prev_prev = cv2.warpPerspective(
            img_prev_prev,
            warp_matrix,
            dsize=(prev_prev_width, prev_prev_height),
            borderValue=self.border_val)
        ...
        img_next_next = cv2.warpPerspective(
            img_next_next,
            warp_matrix,
            dsize=(next_next_width, next_next_height),
            borderValue=self.border_val)
