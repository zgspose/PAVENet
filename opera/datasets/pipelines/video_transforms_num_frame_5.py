...
import random as rd
import mmcv
from mmdet.datasets.pipelines import ResizeV2 as MMDetResize
from mmdet.datasets.pipelines import RandomFlipV2 as MMDetRandomFlip
from mmdet.datasets.pipelines import MulRandomCropV2 as MMDetMulRandomCrop
...
class MulResizeV2(MMDetResize):
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super(MulResizeV2, self).__init__(*args, **kwargs)
    ...
    def __call__(self, results):
        ...
        results = super(MulResizeV2, self).__call__(results)
        self._resize_keypoints(results)
        self._resize_areas(results)
        return results

    def __repr__(self):
        repr_str = super(MulResizeV2, self).__repr__()[:-1] + ', '
        repr_str += f'keypoint_clip_border={self.keypoint_clip_border})'
        return repr_str


@PIPELINES.register_module()
class MulRandomFlipV2(MMDetRandomFlip):
    ...
    def __call__(self, results):
        ...
        results = super(MulRandomFlipV2, self).__call__(results)
        if results['flip']:
            # flip keypoints
            for key in results.get('keypoint_fields', []):
...
@PIPELINES.register_module()
class MulRandomCropV2(MMDetMulRandomCrop):
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super(MulRandomCropV2, self).__init__(*args, **kwargs)
    ...
    def __repr__(self):
        repr_str = super(MulRandomCropV2, self).__repr__()[:-1] + ', '
        repr_str += f'kpt_clip_border={self.kpt_clip_border})'
        return repr_str


@PIPELINES.register_module()
class MulKeypointRandomAffineV2:
    ...
    def __call__(self, results):
        img = results['img']
        height = img[0].shape[0] + self.border[0] * 2
        width = img[0].shape[1] + self.border[1] * 2
        
        
         # Center
        center_matrix = np.eye(3, dtype=np.float32)
        center_matrix[0, 2] = -img[0].shape[1] / 2  # x translation (pixels)
        center_matrix[1, 2] = -img[0].shape[0] / 2  # y translation (pixels)
