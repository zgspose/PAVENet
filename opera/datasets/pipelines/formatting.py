...
from mmdet.datasets.pipelines.formatting import DefaultFormatBundleV2 as MMDetDefaultFormatBundleV2
...
@PIPELINES.register_module()
class DefaultFormatBundleV2(MMDetDefaultFormatBundleV2):
    """Default formatting bundle.
    ...
    def __init__(self,
                 *args,
                 extra_keys=[],
                 **kwargs):
        super(DefaultFormatBundleV2, self).__init__(*args, **kwargs)
        self.extra_keys = extra_keys
    ...
    def __call__(self, results):
        ...
        results = super(DefaultFormatBundleV2, self).__call__(results)
        assert isinstance(self.extra_keys, (list, tuple))
        if self.extra_keys:
            for key in self.extra_keys:
                if key not in results:
