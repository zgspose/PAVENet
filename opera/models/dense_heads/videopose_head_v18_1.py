...
# pose-decoder： 
#           方式：时空解码
#           pose_query = featrue_token + 空间位置编码（可学习） + learnable
#           matchCost = l1Cost + clsCost
#           loss = rle_loss + cls_loss
# joint-decoder:
#           方式：先回归辅助帧和当前帧的keypoint_tokens，再进行时空双分支并行建模
#           joint_query = pose_token + learnable + 空间位置编码（可学习）+ 时间位置编码（可学习）
#           loss = rle_loss + cls_loss
@HEADS.register_module()
class VideoPoseHeadV18_1(AnchorFreeHead):
    ...
    def _init_layers(self):
        ...
        # encoder、pose-deocder使用该子网络进行坐标偏移量的回归
        kpt_branch = []
        ...
        # pose-decoder使用回归关节点的sigma
        pose_fc_sigma_branch = []
        pose_fc_sigma_branch.append(Linear(self.embed_dims, 512))
        for _ in range(self.num_kpt_fcs):
            pose_fc_sigma_branch.append(Linear(512, 512))
        pose_fc_sigma_branch.append(Linear_with_norm(512, 2 * self.num_keypoints, norm=False))
        pose_fc_sigma_branch = nn.Sequential(*pose_fc_sigma_branch)
        ...
        if self.with_kpt_refine:
            ...
            self.pose_fc_sigma_branches = _get_clones(pose_fc_sigma_branch, num_pred - 1)
        ...
        self.query_embedding = nn.Embedding(self.num_query,
                                            self.embed_dims * 2)
        # pose_query 时间位置编码
        self.pose_time_embedding = nn.Embedding(self.num_frames, self.embed_dims) # num_frames, embed_dims
        
        
        # joint-decoder使用该子网络进行坐标偏移量的回归
        refine_kpt_branch = []
        for _ in range(self.num_kpt_fcs):
        ...
        if self.with_kpt_refine:
            ...
            # 最后一层用于融合时空后的关节点tokens
            self.refine_kpt_branches = _get_clones(refine_kpt_branch, num_pred + 1)
            self.refine_fc_sigma_branches = _get_clones(refine_fc_sigma_branch, num_pred + 1) 
        self.fc_hm = Linear(self.embed_dims, self.num_keypoints)
        
        # add decoder flow model
        masks = torch.from_numpy(np.array([[0, 1], [1, 0]] * 3).astype(np.float32))
        
        pose_prior = distributions.MultivariateNormal(torch.zeros(2) + 0.5, torch.eye(2))
        self.pose_flow = RealNVP(nets, nett, masks, pose_prior)
        
        joint_prior = distributions.MultivariateNormal(torch.zeros(2) + 0.5, torch.eye(2))
        self.joint_flow = RealNVP(nets, nett, masks, joint_prior)
    ...
    def forward(self, mlvl_feats, img_metas):
        ...
        query_time_embeds = self.pose_time_embedding.weight # num_frames, embed_dims
        ...
        for lvl in range(hs.shape[0]):
            ...
            sigma = self.pose_fc_sigma_branches[lvl](hs[lvl])
        ...
        if self.as_two_stage:
            return outputs_classes, outputs_kpts, output_sigmas, \
                ...
                hm_proto, memory, mlvl_masks, hs[-1].detach()
        else:
            raise RuntimeError('only "as_two_stage=True" is supported.')
        
    def forward_refine(self, memory, mlvl_masks, pose_tokens, refine_targets, losses,
                       img_metas):
        """Forward function.
        ...
        pose_tokens = pose_tokens.reshape(-1, self.embed_dims) # bs*num_querys, embed_dims
        if pos_inds.sum() == 0:
            pos_kpt_preds = torch.zeros_like(kpt_preds[:1])
            pos_img_inds = kpt_preds.new_zeros([1], dtype=torch.int64)
            pose_tokens = torch.zeros_like(pose_tokens[:1])
        else:
            pos_kpt_preds = kpt_preds[pos_inds]
            pos_img_inds = (pos_inds.nonzero() / self.num_query).squeeze(1).to(
                torch.int64)
            pose_tokens = pose_tokens[pos_inds] # bs*num_querys, embed_dims
        # 将当前帧关节点位置作为辅助帧关节点初始位置
        ...
        ST_output, hs, init_reference, inter_references = self.transformer.forward_refine(
            mlvl_masks,
            memory, # 所有帧feature_tokens
            pose_tokens,
        ...
        tmp_kpt = self.refine_kpt_branches[-1](ST_output[1])
        tmp_sigma = self.refine_fc_sigma_branches[-1](ST_output[1]).sigmoid()
        score = 1 - tmp_sigma
        score = torch.mean(score, dim=2, keepdim=True)
        outputs_score = score
        outputs_scores.append(outputs_score)
        ...
        for i, (kpt_refine_preds, kpt_refine_sigma) in enumerate(zip(outputs_kpts, outputs_sigmas)):
            ...
            log_phi = self.joint_flow.log_prob(bar_mu.reshape(-1, 2)).reshape(bs, -1, 1)
    ...
    def forward_train(self,
        ...
        memory, mlvl_masks, pose_tokens = outs[-3:]
        outs = outs[:-3]
        ...
        losses = self.forward_refine(memory, mlvl_masks, pose_tokens,
    ...
    def loss_single(self,
        ...
        if pos_inds.sum() == 0:
            ...
            log_phi = self.pose_flow.log_prob(bar_mu.reshape(-1, 2)).reshape(num_gt, -1, 1)
    ...
    def get_bboxes(self,
        ...
        for img_id in range(len(img_metas)):
            ...
            proposals = self._get_bboxes_single(cls_score, kpt_pred,
                                                ...
                                                memory, mlvl_masks, pose_tokens, rescale)
    ...
    def _get_bboxes_single(self,
        ...
        if self.loss_cls.use_sigmoid:
            ...
            pose_tokens = pose_tokens.reshape(-1, self.embed_dims)[bbox_index]
        else:
        ...
        refine_outputs = self.forward_refine(memory, mlvl_masks, pose_tokens,
