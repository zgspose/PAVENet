...
# 添加时间 2024-11-9 only pose-decoder 时空解码  适用5帧
# match_loss: cls loss + l1 loss + oks loss
# total_loss: cls loss(pose-decoder + encoder) + l1 loss(encoder) + rle loss(pose-decoder)
@HEADS.register_module()
class VideoPoseHeadV14_2_num_frame_5(AnchorFreeHead):
    ...
    def forward(self, mlvl_feats, img_metas):
        ...
        batch_size = mlvl_feats[0].size(0)  # 这里实际上batch-size大小 为 batch-size // num_frames
    ...
    def _get_bboxes_single(self,
        ...
        det_kpts = kpt_pred.reshape(max_per_img, -1, 2)
        det_kpts_sigmas = sigma_pred.reshape(max_per_img, -1, 2)
