...
# encoder:
#           loss = rle loss
# joint-decoder每帧初始参考点使用pose-decoder的每帧的结果，每层回归采样点偏移+参考点偏移
@HEADS.register_module()
class VideoPoseHeadV11_21_v3(AnchorFreeHead):
    ...
    def _init_layers(self):
        ...
        # 获取pose-decoder中不同帧的偏移回归网络-----------------
        self.pre_kpt_branches = nn.ModuleList()
        # 只需要和pose-decoder层数相同
    ...
    def forward(self, mlvl_feats, img_metas):
        ...
        if self.as_two_stage:
            return outputs_classes, outputs_kpts, output_sigmas, \
                enc_outputs_class, enc_outputs_kpt.sigmoid(),  enc_outputs_sigma.sigmoid(), \
                hm_proto, memory, mlvl_masks, pre_pose_kpt, next_pose_kpt
        else:
            raise RuntimeError('only "as_two_stage=True" is supported.')
    ...
    def forward_refine(self, memory, mlvl_masks, pre_pose_kpt, next_pose_kpt, refine_targets, losses,
        ...
        for i, (kpt_refine_preds, kpt_refine_sigma) in enumerate(zip(outputs_kpts, outputs_sigmas)):
            ...
            if pos_inds.sum() == 0:
                ...
                losses[f'd{i}.loss_kpt_refine'] = loss_kpt # rle loss
            ...
            pred = kpt_refine_preds # bs, num_keypoints, 2
            sigma = kpt_refine_sigma # bs, num_keypoints, 2
    ...
    def get_bboxes(self,
        ...
        for img_id in range(len(img_metas)):
            ...
            proposals = self._get_bboxes_single(cls_score, kpt_pred, pre_pose_kpt, next_pose_kpt,
                                                img_shape, scale_factor,
                                                memory, mlvl_masks, rescale, sigma_pred)
    ...
    def simple_test_bboxes(self, feats, img_metas, rescale=False):
        ...
        # end = time.time()
        # print(f'pose-decoder time: {end - start:.6f}')
