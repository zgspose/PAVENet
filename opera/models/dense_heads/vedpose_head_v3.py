...
# encoder, pose-decoder 与 joint-decoder均使用rle-loss
@HEADS.register_module()
class VedPoseHeadV3(AnchorFreeHead):
    ...
    def forward(self, mlvl_feats, img_metas):
        ...
        for lvl in range(hs.shape[0]):
            ...
            tmp_sigma = self.dec_fc_sigma_branches[lvl](hs[lvl])
            assert reference.shape[-1] == self.num_keypoints * 2
            tmp_kpt += reference
            outputs_kpt = tmp_kpt.sigmoid()
            outputs_sigma = tmp_sigma.sigmoid()
        ...
        if self.as_two_stage:
            # 返回值： pose_decoder的300个pose的类别得分(没有sigmoid)
            return outputs_classes, outputs_kpts, outputs_sigmas, \
                enc_outputs_class, enc_outputs_kpt.sigmoid(), enc_outputs_sigma, \
    ...
    def get_bboxes(self,
        ...
        Returns:
            list[list[Tensor, Tensor]]: Each item in result_list is 3-tuple.
                ...
                p^{K}_v] format.
        """
        cls_scores = all_cls_scores[-1]  # 300个pose的得分，no sigmoid， shape： 1, 300, 1, 只使用decoder-layer的最后一层的输出结果
        kpt_preds = all_kpt_preds[-1] # 300个pose的粗粒度坐标 shape: 1, 300, 30
    ...
    #     max_per_img = self.test_cfg.get('max_per_img', self.num_query)
    #     # exclude background
    #     # 这边需要修改，Spose = Sbbox * mean(sum(Skpt))
    #     if self.loss_cls.use_sigmoid:
    #         cls_score = cls_score.sigmoid()
    #         scores, indexs = cls_score.view(-1).topk(max_per_img)
    #         det_labels = indexs % self.num_classes
    ...
    #     # follow query_pose
    #     det_bboxes_scores = scores * torch.mean(det_kpts_score, dim=1).squeeze(-1)
    #     det_bboxes = torch.cat((det_bboxes, det_bboxes_scores.unsqueeze(1)), -1)
        
    #     # 关节点score需要重新计算, 原始是关节点得分直接为1,参考posewarper
    #     kpt_scores = det_bboxes_scores[:, None, None] * det_kpts_score
    #     # kpt_scores = det_kpts_score
    ...
    #     # nms
    #     keep, _ = oks_nms(det_kpts, det_bboxes_scores, OKS_SCORE, np.array(OKS_SIGMAS) / 10.0)
    #     tmp_det_kpts = []
    #     tmp_det_labels = []
    #     tmp_det_bboxes = []
    #     for keep_ in keep:
    ...
    def simple_test_bboxes(self, feats, img_metas, rescale=False):
        ...
        print(f'bk + encoder + pose time: {end - start:.6f}')
