...
# 添加时间 2024-10-20 适用于只在Pose-decoder中进行时空编码模块
@HEADS.register_module()
class VideoPoseHeadV11(AnchorFreeHead):
    ...
    def forward(self, mlvl_feats, img_metas):
        ...
        hs, init_reference, inter_references, \
            enc_outputs_class, enc_outputs_kpt, hm_proto = \
                self.transformer(
                    mlvl_feats,
                    mlvl_masks,
                    query_embeds,
        ...
        # decoder添加关节点的sigma预测，便于计算rle loss
        ...
        outputs_classes = torch.stack(outputs_classes) # shape： num_layer, num_img, num_query, 1
        outputs_kpts = torch.stack(outputs_kpts) # shape: num_layer, num_img, num_query, num_joints*2
        outputs_sigmas = torch.stack(outputs_sigmas).sigmoid() # shape: num_layer, num_img, num_query, num_joints*2
        ...
        if self.as_two_stage:
            return outputs_classes, outputs_kpts, outputs_sigmas, \
    ...
    def loss(self,
        ...
        # decoder loss 计算 一层一层计算
        losses_cls, losses_kpt = multi_apply(
        ...
        # encoder loss 计算 无修改，继续使用cls loss + l1 loss
    ...
    def loss_single(self,
        ...
        # 关节点回归损失，这里替换为rle loss 原本为l1 loss 
        # 展平
        kpt_preds = kpt_preds.reshape(-1, kpt_preds.shape[-1])
        sigma_preds = sigma_preds.reshape(-1, sigma_preds.shape[-1])
        num_valid_kpt = torch.clamp(
            reduce_mean(kpt_weights.sum()), min=1).item()
        # assert num_valid_kpt == (kpt_targets>0).sum().item()
        
         # 获取与gt对应的pred，便于参与后续rle loss 计算
        pos_inds = kpt_weights.sum(-1) > 0
        if pos_inds.sum() == 0:
    ...
    def _get_bboxes_single(self,
        ...
        det_kpts_score = torch.mean(det_kpts_sigmas, dim=-1, keepdim=True) # shape： 100, 15, 1
        ...
        # p_to_coord_index = 5
        # output_regression = output_regression * output_regression_p**p_to_coord_index
        ...
        # 
        # scores = scores * torch.mean(det_kpts_score, dim=1).squeeze(-1)
