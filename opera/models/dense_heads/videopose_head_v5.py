...
# 添加时间 2024-9-30 适用于fuse-time module
@HEADS.register_module()
class VideoPoseHeadV5(AnchorFreeHead):
    ...
    def _init_layers(self):
        ...
        if self.with_kpt_refine:
            ...
            self.fuse_time_cls_branches = _get_clones(fc_cls, 1)
            self.fuse_time_kpt_branches = _get_clones(kpt_branch, 1)
    ...
    def forward(self, mlvl_feats, img_metas):
        ...
        fuse_time_pose_tokens, hs, init_reference, inter_references, \
        ...
        # TODO start：处理融合时序信息后的pose-token，进行再次回归相对偏移，得到最终的pose位置
        if self.fuse_time_cls_branches:
            fuse_time_pose_tokens = fuse_time_pose_tokens.transpose(0, 1)[1::3]
            reference = inverse_sigmoid(outputs_kpts[-1].detach())
            outputs_class = self.fuse_time_cls_branches[0](fuse_time_pose_tokens)
            tmp_kpt = self.fuse_time_kpt_branches[0](fuse_time_pose_tokens)
            assert reference.shape[-1] == self.num_keypoints * 2
            tmp_kpt += reference
            outputs_kpt = tmp_kpt.sigmoid()
            outputs_classes.append(outputs_class)
        ...
        if self.as_two_stage:
            ...
            return outputs_classes, outputs_kpts, \
                ...
                hm_proto, memory, new_mlvl_masks
        else:
            raise RuntimeError('only "as_two_stage=True" is supported.')
