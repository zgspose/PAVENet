...
# 添加时间 2024-10-22 pose-decoder 时空解码 增加新的joint-decoder
# match_loss: cls loss + l1 loss
# total_loss: pose-decoder: cls loss + rle loss joint-decoder: rle loss
@HEADS.register_module()
class VideoPoseHeadV16(AnchorFreeHead):
    ...
    def __init__(self,
                 ...
                 loss_kpt_refine=dict(type='OKSLoss', loss_weight=2.0),
        ...
        # TODO
        self.regress_offsets = regress_offsets
    ...
    def _init_layers(self):
        ...
        # pose-decoder用于预测关节点的sigma
        dec_sigma_branch = []
        for _ in range(self.num_kpt_fcs):
            dec_sigma_branch.append(Linear(self.embed_dims, self.embed_dims))
        dec_sigma_branch.append(Linear_with_norm(self.embed_dims, 2 * self.num_keypoints, norm=False))
        dec_sigma_branch = nn.Sequential(*dec_sigma_branch)
        
        # joint用于预测关节点的sigma
        joint_sigma_branch = []
        for _ in range(self.num_kpt_fcs):
            joint_sigma_branch.append(Linear(self.embed_dims, self.embed_dims))
        joint_sigma_branch.append(Linear_with_norm(self.embed_dims, 2, norm=False))
        joint_sigma_branch = nn.Sequential(*joint_sigma_branch)
        ...
        if self.with_kpt_refine:
            ...
            self.dec_sigma_branches = _get_clones(dec_sigma_branch, num_pred)
            self.joint_sigma_branches = _get_clones(joint_sigma_branch, 1)
            self.refine_joint_branches = _get_clones(refine_kpt_branch, 1)
        else:
    ...
    def forward(self, mlvl_feats, img_metas):
        ...
        joint_tokens, hs, init_reference, inter_references, \
        ...
        for lvl in range(hs.shape[0]):
            ...
            sigma = self.dec_sigma_branches[lvl](hs[lvl])
            ...
            outputs_sigmas.append(output_sigma)
        ...
        return outputs_classes, outputs_kpts, outputs_sigmas, \
            ...
            hm_proto, joint_tokens, memory, mlvl_masks
            
    def forward_refine(self, joint_tokens, memory, mlvl_masks, refine_targets, losses,
                       img_metas):
        """Forward function.
        ...
        # 1.使用joint-extract模块提取对应gt的预测结果对应的关节点特征token
        # 2.使用时空双分支分别进一步融合时空信息
        # shape: bs, num_frames, num_query, num_keypoints, embed_dims
        fuse_joint_tokens = self.transformer.forward_refine(joint_tokens)
        # 取出对应样本的当前帧的joint-tokens, flatten
        # 3.提取当前帧的时间和空间信息，融合时间和空间信息进行最后的关节点回归
        fuse_joint_tokens = fuse_joint_tokens[:, 1::3].flatten(0, 2)[pos_inds]
        
        reference = pos_kpt_preds.detach().reshape(-1, self.num_keypoints, 2)

        outputs_kpts = []
        outputs_sigmas = []
        outputs_scores = []
        if self.regress_offsets:
            reference = inverse_sigmoid(reference)
            tmp_kpt = self.refine_joint_branches[0](fuse_joint_tokens)
            tmp_sigma = self.joint_sigma_branches[0](fuse_joint_tokens).sigmoid()
            score = 1 - tmp_sigma
        ...
        # 修改，将原始的l1 loss修改为rle loss,这里输出的joint-decoder_layer共有两层
        for i, (kpt_refine_preds, kpt_refine_sigma) in enumerate(zip(outputs_kpts, outputs_sigmas)):
            # 如果预测的结果为空，没有人的情况下
            if pos_inds.sum() == 0:
                loss_kpt = kpt_refine_preds.sum() * 0
    ...
    def forward_train(self,
        ...
        joint_tokens, memory, mlvl_masks = outs[-3:]
        outs = outs[:-3]
        ...
        losses = self.forward_refine(joint_tokens, memory, mlvl_masks, 
    ...
    def loss_single(self,
        ...
        # kpt rle loss
        # 处理一下，这里计算loss只需要和gt对应的kpt_pred 和 sigma_pred
        pos_inds = kpt_weights.sum(-1) > 0
        if pos_inds.sum() == 0:
            ...
            # reshape为 num_gt, num_keypoints, 2
        ...
        return loss_cls, loss_kpt, kpt_preds, kpt_targets, \
    ...
    def get_bboxes(self,
        ...
        for img_id in range(len(img_metas)):
            ...
            proposals = self._get_bboxes_single(cls_score, kpt_pred,
                                                ...
                                                joint_tokens, memory, mlvl_masks, rescale)
    ...
    def _get_bboxes_single(self,
        ...
        refine_outputs = self.forward_refine(joint_tokens, memory, mlvl_masks,
