...
class VideoPoseHeadV17_5_test(AnchorFreeHead):
    ...
    def _init_layers(self):
        ...
        if self.with_kpt_refine:
            ...
            self.pre_kpt_branches = _get_clones(kpt_branch, num_pred - 1)
            self.kpt_branches = _get_clones(kpt_branch, num_pred)
            self.next_kpt_branches = _get_clones(kpt_branch, num_pred - 1)
            self.dec_fc_sigma_branches = _get_clones(dec_fc_sigma_branch, num_pred - 1)
        ...
        if self.with_kpt_refine:
            ...
            self.pre_refine_kpt_branches = _get_clones(refine_kpt_branch, num_pred)
            self.refine_kpt_branches = _get_clones(refine_kpt_branch, num_pred)
            self.next_refine_kpt_branches = _get_clones(refine_kpt_branch, num_pred)
        ...
        # # 固定encoder和pose-decoder中的回归网络参数
        # for cls in self.cls_branches:
        #     for param in cls.parameters():
        #         param.requires_grad = False
        # for kpt in self.kpt_branches:
        #     for param in kpt.parameters():
        #         param.requires_grad = False
        # for sigma in self.pose_fc_sigma_branches:
        #     for param in sigma.parameters():
        #         param.requires_grad = False
        # for param in self.pose_flow.parameters():
        #     param.requires_grad = False
    ...
    def forward(self, mlvl_feats, img_metas):
        ...
        if self.as_two_stage:
            return outputs_classes, outputs_kpts, output_sigmas, \
                ...
                hm_proto, memory, mlvl_masks, pre_kpt, next_kpt
        else:
            raise RuntimeError('only "as_two_stage=True" is supported.')
        
    def forward_refine(self, memory, mlvl_masks, pre_pose_kpt, next_pose_kpt, refine_targets, losses,
                       img_metas):
        """Forward function.
    ...
    def forward_train(self,
        ...
        memory, mlvl_masks, pre_pose_kpt, next_pose_kpt = outs[-4:]
        outs = outs[:-4]
        ...
        losses = self.forward_refine(memory, mlvl_masks, pre_pose_kpt, next_pose_kpt,
    ...
    def get_bboxes(self,
        ...
        for img_id in range(len(img_metas)):
            ...
            proposals = self._get_bboxes_single(cls_score, kpt_pred,sigma_pred, pre_pose_kpt, next_pose_kpt,
    ...
    def _get_bboxes_single(self,
        ...
        # refine_outputs = self.forward_refine(memory, mlvl_masks, pre_pose_kpt, next_pose_kpt,
