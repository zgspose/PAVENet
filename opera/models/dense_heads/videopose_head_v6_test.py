...
# 添加时间 2024-10-6 适用于只在Pose-decoder中进行时空编码模块
@HEADS.register_module()
class VideoPoseHeadV6(AnchorFreeHead):
    ...
    def __init__(self,
                 num_classes,
                 in_channels,
                 # 控制是否在joint-decoder使用时空解码
                 joint_space_time_decoder=False,
                 num_query=100,
                 num_kpt_fcs=2,
                 num_keypoints=17,
                 transformer=None,
        ...
        # 添加
        if joint_space_time_decoder:
            self.joint_space_time_decoder = True
        else:
            self.joint_space_time_decoder =False
        self._init_layers()
    ...
    def forward(self, mlvl_feats, img_metas):
        ...
        if self.as_two_stage:
            if self.joint_space_time_decoder:
