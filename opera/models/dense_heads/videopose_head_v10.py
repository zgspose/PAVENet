...
# 添加时间 2024-10-19 适用于Pose-decoder中进行时空编码模块，无joint时序，pose-decoder也使用rle 代替原始的l1 loss
@HEADS.register_module()
class VideoPoseHeadV10(AnchorFreeHead):
    ...
    def __init__(self,
        ...
        # TODO 使用rle-loss 替换原始的 l1-loss
        self.enc_rle_loss = enc_rle_loss
        self.dec_rle_loss = dec_rle_loss
        if self.enc_rle_loss or self.dec_rle_loss:
            self.use_rle_loss = True
        else:
            self.use_rle_loss = False
        masks = torch.from_numpy(np.array([[0, 1], [1, 0]] * 3).astype(np.float32))
        # 添加rle损失计算需要的流模型网络
        if self.enc_rle_loss:
            enc_prior = distributions.MultivariateNormal(torch.zeros(2) + 0.5, torch.eye(2))
            self.enc_flow = RealNVP(nets, nett, masks, enc_prior)
        else:
            self.enc_flow = None
        if self.dec_rle_loss:
    ...
    def _init_layers(self):
        ...
        # encoder、pose-decoder所使用分类子网络
        fc_cls = Linear(self.embed_dims, self.cls_out_channels)

        # encoder、pose-deocder所使用回归关节点以及偏移子网络
        kpt_branch = []
        ...
        if self.use_rle_loss:
            ...
            fc_sigma_branch.append(Linear_with_norm(self.embed_dims, 2*self.num_keypoints, norm=False))
    ...
    def forward(self, mlvl_feats, img_metas):
        ...
        hs, init_reference, inter_references, \
            enc_outputs_class, enc_outputs_kpt, enc_outputs_sigma, hm_proto = \
                self.transformer(
                    mlvl_feats,
                    mlvl_masks,
                    query_embeds,
                    ...
                    sigmas_branches=self.fc_sigma_branches
            )
        ...
        dec_outputs_classes = outputs_classes
        dec_outputs_sigma = outputs_sigmas.sigmoid()
        dec_outputs_kpt = outputs_kpts
        ...
        # dec_outputs = EasyDict(
        #     pred_jts=outputs_kpts,
        #     sigma=outputs_sigmas,
        ...
        return dec_outputs_classes, dec_outputs_kpt, dec_outputs_sigma, \
               enc_outputs_class, enc_outputs_kpt, enc_outputs_sigma, \
               hm_proto
    ...
    def forward_refine(self, memory, mlvl_masks, refine_targets, losses,
        ...
        # TODO 修改对应的数据 ----  训练样本中pos归属 + 前后帧pos坐标分配
        new_pos_img_inds = []
        # 统计每个训练样本中包含人的个数
        counter = Counter(pos_img_inds.cpu().numpy())
        for num, count in counter.items():
            mid_v = num * 3 
            new_pos_img_inds.extend([mid_v, mid_v+1, mid_v+2]*count)
        pos_img_inds = torch.tensor(new_pos_img_inds)
        pos_kpt_preds =  pos_kpt_preds.repeat_interleave(3, dim=0)
        
        # TODO end
        last_joint_tokens, hs, init_reference, inter_references = self.transformer.forward_refine(
            mlvl_masks,
            memory,
        ...
        for lvl in range(hs.shape[0]):
            ...
            tmp_sigma = self.refine_fc_sigma_branches[lvl](hs[lvl]).sigmoid().reshape(-1, 3, self.num_keypoints, 2)[:, 1]
            score = 1 - tmp_sigma
            ...
            tmp_kpt += reference
            outputs_kpt = tmp_kpt.sigmoid().reshape(-1, 3, self.num_keypoints, 2)[:, 1]
            outputs_kpts.append(outputs_kpt)
            
            outputs_sigma = tmp_sigma
            outputs_sigmas.append(outputs_sigma)
            
        # 处理时序融合后的joint-tokens
        reference = inverse_sigmoid(outputs_kpts[-1])
        tmp_kpt = self.refine_kpt_branches[-1](last_joint_tokens)
        tmp_sigma = self.refine_fc_sigma_branches[-1](last_joint_tokens).sigmoid()
        score = 1 - tmp_sigma
        score = torch.mean(score, dim=2, keepdim=True)
        outputs_score = score
        outputs_scores.append(outputs_score)
        ...
        # 修改，将原始的l1loss修改为rleloss,这里输出的decoder_layer共有两层
    ...
    def loss(self,
        ...
        # decoder相关损失计算
        losses_cls, losses_kpt, kpt_preds_list, sigma_preds_list, kpt_targets_list, \
            area_targets_list, kpt_weights_list = multi_apply(
                self.loss_single, all_cls_scores, all_kpt_preds, all_sigmas,
                all_gt_labels_list, all_gt_keypoints_list,
                all_gt_areas_list, img_metas_list)

        loss_dict = dict()
        
        # encoder 中的proposal相关损失计算
        ...
        if enc_cls_scores is not None:
            ...
            enc_loss_cls, enc_losses_kpt = \
                self.loss_single_rpn(
                    enc_cls_scores, enc_kpt_preds, enc_sigmas, binary_labels_list,
        ...
        return loss_dict, (kpt_preds_list[-1], sigma_preds_list[-1], kpt_targets_list[-1],
    ...
    def loss_single(self,
        ...
        sigmas_preds_list = [sigmas_preds[i] for i in range(num_imgs)]
        flow_model = [self.dec_flow for i in range(num_imgs)]
        
        cls_reg_targets = self.get_targets(cls_scores_list, kpt_preds_list, sigmas_preds_list,
                                           gt_labels_list, gt_keypoints_list,
                                           gt_areas_list, img_metas, flow_model)
        # todo
        ...
        sigmas_preds = sigmas_preds.reshape(-1, sigmas_preds.shape[-1])
        num_valid_kpt = torch.clamp(
            reduce_mean(kpt_weights.sum()), min=1).item()
        # assert num_valid_kpt == (kpt_targets>0).sum().item()
        
        # 处理一下，只要和gt对应的kpt_pred 和 sigma_pred
        pos_inds = kpt_weights.sum(-1) > 0
        if pos_inds.sum() == 0:
            kpt_preds = torch.zeros_like(kpt_preds[:1])
            print('可能存在问题')
            return kpt_preds.sum() * 0, kpt_preds.sum() * 0, sigmas_preds, kpt_preds, kpt_targets, \
            area_targets, kpt_weights
        else:
            kpt_preds = kpt_preds[pos_inds]
        kpt_targets = kpt_targets[pos_inds] # num_gt, 30
        kpt_weights = kpt_weights[pos_inds] # num_gt, 30
        sigmas_preds = sigmas_preds[pos_inds] # num_gt, 30
        # kpt rle loss
        # reshape 数据
        num_gt = kpt_preds.size(0)
        all_gts = kpt_targets.reshape(num_gt, -1, 2)
        gt_weights = kpt_weights.reshape(num_gt, -1, 2)
        pred = kpt_preds.reshape(num_gt, -1, 2)
        ...
        loss_kpt = self.loss_kpt(output, labels)

        return loss_cls, loss_kpt, kpt_preds, sigmas_preds, kpt_targets, \
            area_targets, kpt_weights
    ...
    def get_targets(self,
        ...
        (labels_list, label_weights_list, kpt_targets_list, kpt_weights_list,
         area_targets_list, pos_inds_list, neg_inds_list) = multi_apply(
             self._get_target_single, cls_scores_list, kpt_preds_list, sigma_pred_list,
             gt_labels_list, gt_keypoints_list, gt_areas_list, img_metas, flow_model)
    ...
    def _get_target_single(self,
        ...
        assign_result = self.assigner.assign(cls_score, kpt_pred, sigma_pred, gt_labels,
                                             gt_keypoints, gt_areas, img_meta, flow_model)
        ...
        # kpt_sigmas = torch.zeros_like(kpt_pred) # 300, 15, 2
        
        pos_gt_kpts = gt_keypoints[sampling_result.pos_assigned_gt_inds] # 预测结果与gt对应的索引
    ...
    def loss_single_rpn(self,
        ...
        cls_scores_list = [cls_scores[i] for i in range(num_imgs)]
        kpt_preds_list = [kpt_preds[i] for i in range(num_imgs)]
        sigma_preds_list = [sigma_preds[i] for i in range(num_imgs)]
        flow_model = [self.enc_flow for i in range(num_imgs)]
        
        cls_reg_targets = self.get_targets(cls_scores_list, kpt_preds_list, sigma_preds_list,
                                           gt_labels_list, gt_keypoints_list,
                                           gt_areas_list, img_metas, flow_model)
        (labels_list, label_weights_list, kpt_targets_list, kpt_weights_list,
         area_targets_list, num_total_pos, num_total_neg) = cls_reg_targets
        ...
        # num_total_pos = torch.clamp(reduce_mean(num_total_pos), min=1).item()
         ...
         # keypoint regression loss
        kpt_preds = kpt_preds.reshape(-1, kpt_preds.shape[-1])
        sigmas_preds = sigma_preds.reshape(-1, sigma_preds.shape[-1])
        num_valid_kpt = torch.clamp(
            reduce_mean(kpt_weights.sum()), min=1).item()
        # assert num_valid_kpt == (kpt_targets>0).sum().item()
        ...
        if pos_inds.sum() == 0:
            kpt_preds = torch.zeros_like(kpt_preds[:1])
            print('可能存在问题')
            return kpt_preds.sum() * 0, kpt_preds.sum() * 0
        else:
            kpt_preds = kpt_preds[pos_inds]
        kpt_targets = kpt_targets[pos_inds]
        kpt_weights = kpt_weights[pos_inds]
        sigmas_preds = sigmas_preds[pos_inds]
        # kpt rle loss
        bs = kpt_preds.size(0)
        all_gts = kpt_targets.reshape(bs, -1, 2)
        gt_weights = kpt_weights.reshape(bs, -1, 2)
        pred = kpt_preds.reshape(bs, -1, 2)
        sigmas_preds = sigmas_preds.reshape(bs, -1, 2)
        sigmas = sigmas_preds
        bar_mu = (pred - all_gts) / sigmas
        log_phi = self.enc_flow.log_prob(bar_mu.reshape(-1, 2)).reshape(bs, -1, 1)
        nf_loss = torch.log(sigmas) - log_phi 
        pred_jts = pred
        sigma = sigmas
        output = EasyDict(
        ...
        loss_kpt = self.loss_kpt_rpn(output, labels)
    ...
    def _get_bboxes_single(self,
        ...
        """
        OKS_SCORE = 0.5
        OKS_SIGMAS = [.26, .79, .79, .79, .79, .72, .72, .62,.62, 1.07, 1.07, .87, .87, .89, .89]
        ...
        if self.loss_cls.use_sigmoid:
            ...
            sigma_pred = sigma_pred[bbox_index]
        else:
        ...
        det_kpts = kpt_pred.reshape(max_per_img, self.num_keypoints, -1)
        det_kpts_sigmas = sigma_pred.reshape(det_kpts.shape) # shape: 100, 15, 2
        det_kpts_score = torch.mean(sigma_pred, dim=-1, keepdim=True) # shape： 100, 15, 1
