...
# encoder:
#           loss = l1 loss
# joint-decoder初始参考点使用pose-decoder的当前帧的结果
@HEADS.register_module()
class VideoPoseHeadV11_9_3_freeze(AnchorFreeHead): # 日期 + 版本号
    ...
    def forward_refine(self, memory, mlvl_masks, refine_targets, losses,
        ...
        if pos_inds.sum() == 0:
            ...
            pos_img_inds = (pos_inds.nonzero() / self.num_query).squeeze(1).to(
                torch.int64)
        # 只需要当前帧数据
        new_mlvl_masks = [mlvl_mask[1:2] for mlvl_mask in mlvl_masks]
