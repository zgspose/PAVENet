...
# 添加时间 2024-8-15 适用于pose-tracking 模块   添加辅助帧提取pose-token中的self-attn
@HEADS.register_module()
class VideoPoseHeadV4(AnchorFreeHead):
    ...
    def _init_layers(self):
        ...
        # pose-tracking module 3次cross,1次最终回归
        self.pose_tracking_kpt_branches = _get_clones(kpt_branch, 4)
        self.pose_tracking_time_embedding = nn.Embedding(3, self.embed_dims)
        ...
        if self.with_kpt_refine:
            ...
            # self.pose_tracking_sigma_branches = _get_clones(refine_fc_sigma_branch, 1)
            self.refine_kpt_branches = _get_clones(refine_kpt_branch, num_pred+1)  # 修改，增加一个回归模块用于回归融合时序后的当前帧的pose-kpt
            # 用于预测关节点的sigma网络
            self.refine_fc_sigma_branches = _get_clones(refine_fc_sigma_branch, num_pred+1) # 修改，增加一个回归模块用于回归融合时序后的当前帧的pose-sigmas
    ...
    def forward(self, mlvl_feats, img_metas):
        ...
        hs, init_reference, inter_references, \
            ...
            mask_flatten, spatial_shapes, level_start_index, valid_ratios= \
                self.transformer(
                    mlvl_feats,
                    mlvl_masks,
                    query_embeds,
        ...
        if self.as_two_stage:
            return outputs_classes, outputs_kpts, \
                ...
                hs[-1], mask_flatten, spatial_shapes, level_start_index, valid_ratios  # pose-tracking module 需要使用
        else:
            raise RuntimeError('only "as_two_stage=True" is supported.')

    def forward_refine(self, memory, mlvl_masks, 
                       now_frame_pose_token, mask_flatten, spatial_shapes, level_start_index, valid_ratios,
                       refine_targets, losses,
                       img_metas,
                       bbox_index=None): # pose-tracking ues -- test mode
        """Forward function.
        ...
        pos_inds = kpt_weights.sum(-1) > 0 # shape: bs*num_query
        if pos_inds.sum() == 0:
            ...
            now_frame_pose_token = now_frame_pose_token.flatten(0, 1)[pos_inds] # 根据索引取出gt数量的pose-token,stack到一起，shape: num_gt, dims
            num_pose_per_batch = pos_kpt_preds.shape[0] # batch中pose/gt的个数
            if self.training:
                now_frame_pose_query_pos, now_frame_pose_query = torch.split(self.query_embedding.weight.repeat(pos_inds.shape[0]//300, 1)[pos_inds], self.embed_dims, -1) # 根据索引获取与任务相关的pose-query, 个数同样和gt个数一致 shape: num_gt, dims
            else:
                now_frame_pose_query_pos, now_frame_pose_query = torch.split(self.query_embedding.weight.repeat(pos_inds.shape[0]//100, 1)[bbox_index], self.embed_dims, -1) # 根据索引获取与任务相关的pose-query, 个数同样和gt个数一致 shape: num_gt, dims
            prev_frame_pose_query = next_frame_pose_query = now_frame_pose_query # 复制两份分别作为单样本中前后帧中pose-query shape: num_gt, dims
            # prev_frame_pose_query = next_frame_pose_query = now_frame_pose_token # 复制两份分别作为单样本中前后帧中pose-query shape: num_gt, dims
            
            pos_img_inds = (pos_inds.nonzero() / self.num_query).squeeze(1).to(
                torch.int64)
        # 进行pose-tracking 模块处理 -------------  待实现
        # 流程如下：
        # 1.根据每个样本的300个pose的预测kpt得到与gt对应的pose-token索引
        # 2.根据索引取出gt数量的pose-token,stack到一起，shape: num_gt, dims
        # 3.根据索引获取与任务相关的pose-query, 个数同样和gt个数一致
        # 4.复制两份分别作为单样本中前后帧中pose-query shape: num_gt, 2, dims
        # 5.以每个样本的当前帧得到的与gt对应的pose-token的预测kpt作为参考点复制两份作为前后帧pose-query的参考点
        # 统计每个训练样本中包含人的个数
        counter = Counter(pos_img_inds.cpu().numpy())
        pose_tracking_pose_kpt_preds = torch.stack([pos_kpt_preds.detach().clone(), pos_kpt_preds.detach().clone()], dim=1)  # 前一帧的所有样本中pose的参考点， 后一帧的所有样本中pose的参考点 shape: person_num, 2, 30
        
        prev_memory = []
        next_memory = []
        now_mask_flatten = []
        now_valid_ratios = []
        pose_tracking_mask_flatten = []
        pose_tracking_valid_ratios = []
        
        tmp_prev_memory = memory[0::3] # 所有样本中的前一帧的feature_map
        tmp_next_memory = memory[2::3] # 所有样本中的后一帧的feature_map
        tmp_now_mask_flatten = mask_flatten[1::3] # 所有样本中的当前帧的mask_flatten
        tmp_now_valid_ratios = valid_ratios[1::3] # 所有样本中的当前帧的valid_ratios
    
        tag = 0
        for num, count in counter.items(): # 所有样本中pose的个数 eg: 共四个样本，每个样本包含三帧的feature_map, 四个样本中pose个数分别为 x, y, z, w
            prev_memory.extend([tmp_prev_memory[tag]] * count) # 得到所有样本中前一帧中每个pose对应的feature_map
            next_memory.extend([tmp_next_memory[tag]] * count) # 得到所有样本中后一帧中每个pose对应的feature_map
            now_mask_flatten.extend([tmp_now_mask_flatten[tag]] * count)
            now_valid_ratios.extend([tmp_now_valid_ratios[tag]] * count)
            tag = tag + 1
        
        now_mask_flatten = torch.stack(now_mask_flatten, dim=0)
        pose_tracking_mask_flatten = torch.stack([now_mask_flatten, now_mask_flatten], dim=1)
        
        prev_memory = torch.stack(prev_memory, dim=0)
        next_memory = torch.stack(next_memory, dim=0)
        pose_tracking_memory = torch.stack([prev_memory, next_memory], dim=1) # 堆叠
        
        now_valid_ratios = torch.stack(now_valid_ratios, dim=0)
        pose_tracking_valid_ratios = torch.stack([now_valid_ratios, now_valid_ratios], dim=1)
        
        pose_tracking_time_query_pos = self.pose_tracking_time_embedding.weight # frame_num, dims
        
        hs, ref = self.transformer.pose_tracking(
            query = torch.stack([prev_frame_pose_query, now_frame_pose_token, next_frame_pose_query], dim=1), # num_pose_query, bs, dims
            key=None,
            value=pose_tracking_memory.flatten(0, 1).transpose(0, 1),
            # query_pos=torch.stack([now_frame_pose_query_pos, now_frame_pose_query_pos, now_frame_pose_query_pos], dim=1).flatten(0, 1)[None],
            query_pos = now_frame_pose_query_pos, # single_frame_num_pose, dims
            query_time_pos= pose_tracking_time_query_pos,
            key_padding_mask=pose_tracking_mask_flatten.flatten(0, 1),
            reference_points=pose_tracking_pose_kpt_preds.flatten(0, 1)[:, None],
            spatial_shapes=spatial_shapes,
            level_start_index=level_start_index,
            valid_ratios=pose_tracking_valid_ratios.flatten(0, 1),
            kpt_branches=self.pose_tracking_kpt_branches,
            counter=counter)
        
        ref = ref.squeeze(dim=0)
        
        # prev_pose_reference_points = ref[0]
        # now_pose_reference_points = ref[0]
        # next_pose_reference_points = ref[2]
        
        hs, init_reference, inter_references = self.transformer.forward_refine(
            # 当前帧
            [mlvl_mask[1::3] for mlvl_mask in mlvl_masks],
            # 当前帧
            memory[1::3],
            ref,
            pos_img_inds,
        ...
        batch_size = mlvl_masks[0].size(0) // 3
        factors = []
        for img_id in range(batch_size):
    ...
    def forward_train(self,
        ...
        memory, mlvl_masks, now_frame_pose_token, mask_flatten, spatial_shapes, level_start_index, valid_ratios = outs[-7:]
        outs = outs[:-7]
        ...
        losses = self.forward_refine(memory, mlvl_masks, 
                                     # pose-tracking use
                                     now_frame_pose_token, mask_flatten, spatial_shapes, level_start_index, valid_ratios,
    ...
    def get_bboxes(self,
                   ...
                   memory,
                   mlvl_masks,
                   now_frame_pose_token, mask_flatten, spatial_shapes, level_start_index, valid_ratios, # pose-tracking module use
                   img_metas,
                   rescale=False):
    ...
    def _get_bboxes_single(self,
        ...
        if self.loss_cls.use_sigmoid:
            ...
            # add
            now_frame_pose_token = now_frame_pose_token[0, bbox_index][None]
        else:
        ...
        refine_outputs = self.forward_refine(memory, mlvl_masks,
                                             now_frame_pose_token, mask_flatten, spatial_shapes, level_start_index, valid_ratios, 
                                             refine_targets, None, None, bbox_index)
        # end = time.time()
