...
class VideoPoseHeadV9(AnchorFreeHead):
    ...
    def _init_layers(self):
        ...
        if self.with_kpt_refine:
            self.cls_branches = _get_clones(fc_cls, num_pred+1)
            self.kpt_branches = _get_clones(kpt_branch, num_pred+1)
    ...
    def forward(self, mlvl_feats, img_metas):
        ...
        last_pose_tokens, hs, init_reference, inter_references, \
        ...
        for lvl in range(hs.shape[0]):
            ...
            outputs_class = self.cls_branches[lvl](hs[lvl])[1::3]
            tmp_kpt = self.kpt_branches[lvl](hs[lvl])
            assert reference.shape[-1] == self.num_keypoints * 2
            tmp_kpt += reference
            outputs_kpt = tmp_kpt.sigmoid()[1::3]
            outputs_classes.append(outputs_class)
            outputs_kpts.append(outputs_kpt)
            
        # 处理融合时空pose
        reference = inverse_sigmoid(outputs_kpts[-1])
        outputs_class = self.cls_branches[-1](last_pose_tokens)
        tmp_kpt = self.kpt_branches[-1](last_pose_tokens)
        assert reference.shape[-1] == self.num_keypoints * 2
        tmp_kpt += reference
