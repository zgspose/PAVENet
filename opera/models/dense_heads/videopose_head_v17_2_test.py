...
# joint-decoder:
#           方式：时空解码
#           joint_query = 空间位置编码（可学习）+ learnable
#           loss = rle_loss + cls_loss
@HEADS.register_module()
class VideoPoseHeadV17_2_test(AnchorFreeHead):
    ...
    def _get_bboxes_single(self,
        ...
        if self.loss_cls.use_sigmoid:
            ...
            # 测试pose-decoder直接输出的关节点坐标以及对应的置信度
        ...
        det_kpts = kpt_pred.reshape(-1, self.num_keypoints, 2)
        det_kpts_sigmas = sigma_pred.reshape(-1, self.num_keypoints, 2)
        det_kpts_score = 1 - det_kpts_sigmas
        # 计算score
        det_kpts_score = torch.mean(det_kpts_score, dim=2, keepdim=True)
