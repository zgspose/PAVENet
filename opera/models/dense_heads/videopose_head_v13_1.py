...
# 添加时间 2024-10-21 
# pose-decoder为时空编码
# macth: rle_loss + cls_loss
# 损失： rle_loss + cls_loss
# joint-decoder单帧， loss = rle_loss
@HEADS.register_module()
class VideoPoseHeadV13_1(AnchorFreeHead):
    ...
    def __init__(self,
                 ...
                 loss_kpt_refine=dict(type='L1Loss', loss_weight=70.0),
                 as_two_stage=True,
        ...
        if train_cfg:
            ...
            # pose-decoder使用
            dec_assigner = train_cfg['dec_assigner']
            ...
            self.dec_assigner = build_assigner(dec_assigner)
    ...
    def _init_layers(self):
        ...
        if self.with_kpt_refine:
            ...
            self.fc_sigma_branches = _get_clones(fc_sigma_branch, num_pred -1)
        else:
    ...
    def forward(self, mlvl_feats, img_metas):
        ...
        new_mlvl_masks = [mlvl_mask[1::3] for mlvl_mask in mlvl_masks]
        new_memory = memory[:, 1::3]
        return outputs_classes, outputs_kpts, outputs_sigmas, \
                enc_outputs_class, enc_outputs_kpt.sigmoid(), \
                hm_proto, new_memory, new_mlvl_masks
    ...
    def loss_single(self,
        ...
        flow_model = [self.flow for i in range(num_imgs)]
        ...
        if pos_inds.sum() == 0:
            ...
            log_phi = self.flow.log_prob(bar_mu.reshape(-1, 2)).reshape(num_gt, -1, 1)
    ...
    def _get_target_single(self,
        ...
        if flow_model:
            assign_result = self.dec_assigner.assign(cls_score, kpt_pred, sigma_pred, gt_labels,
    ...
    def loss_single_rpn(self,
        ...
        cls_scores_list = [cls_scores[i] for i in range(num_imgs)]
        kpt_preds_list = [kpt_preds[i] for i in range(num_imgs)]
        sigmas_preds_list = [None for i in range(num_imgs)]
        flow_model_list = [None for i in range(num_imgs)]
        
        cls_reg_targets = self.get_targets(cls_scores_list, kpt_preds_list, sigmas_preds_list,
                                           gt_labels_list, gt_keypoints_list,
                                           gt_areas_list, img_metas, flow_model_list)
        (labels_list, label_weights_list, kpt_targets_list, kpt_weights_list,
         area_targets_list, num_total_pos, num_total_neg) = cls_reg_targets
        labels = torch.cat(labels_list, 0)
        label_weights = torch.cat(label_weights_list, 0)
    ...
    def _get_bboxes_single(self,
        ...
        det_kpts_score = 1 - det_kpts_sigmas
        det_kpts_score = torch.mean(det_kpts_score, dim=-1, keepdim=True) # shape： 100, 15, 1
