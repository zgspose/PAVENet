...
class BaseDetector(BaseModule, metaclass=ABCMeta):
    ...
    def forward(self, img, img_metas, return_loss=True, **kwargs):
        ...
        if return_loss:
            ...
            #         # 获取当前帧
            #         img = data[sample_idx, frame_idx]
            #         img_min = img.min()
            #         img_max = img.max()
            #         img_normalized = (img - img_min) / (img_max - img_min)
            #         img_normalized = img_normalized.permute(1,2,0).cpu().numpy()
            #         img_normalized = (img_normalized * 255).astype(np.uint8)
            #         from PIL import Image
            #         img = Image.fromarray(img_normalized)
                    
            #         # 保存图片
            #         img_filename = os.path.join(sample_dir, f"frame_{frame_idx}.png")
