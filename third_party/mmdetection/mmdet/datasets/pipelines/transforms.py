    ...
    def __call__(self, results):
        ...
        if results['flip']:
            ...
            for key in results.get('img_fields', ['img']):
                ...
                for image in results[key]:
                    image = mmcv.imflip(
                    image, direction=results['flip_direction'])
                    images.append(image)
        ...
        for key in results.get('img_fields', ['img']):
            ...
            if self.size is not None:
                ...
                for image in results[key]:
                    ...
                    image, self.size_divisor, pad_val=pad_val)
                    images.append(image)
                padded_img = np.stack(images, axis=0)
            results[key] = padded_img
...
class NormalizeV2:
    ...
    def __call__(self, results):
        ...
        for key in results.get('img_fields', ['img']):
            images = []
            for image in results[key]:
                image  = mmcv.imnormalize(image, self.mean, self.std,
                                            self.to_rgb)
                images.append(image)
            results[key] = np.stack(images, axis=0)
        results['img_norm_cfg'] = dict(
            mean=self.mean, std=self.std, to_rgb=self.to_rgb)
        return results
...
class MulRandomCrop:
    ...
    def _crop_data(self, results, crop_size, allow_negative_crop):
        ...
        assert crop_size[0] > 0 and crop_size[1] > 0
        # 确保多帧裁剪区域一致
        img_prev = results['img_prev']
        img_now = results['img_now']
        img_next = results['img_next']
        
        
        # 偏移统一
        margin_h = max(img_now.shape[0] - crop_size[0], 0)
        margin_w = max(img_now.shape[1] - crop_size[1], 0)
        offset_h = np.random.randint(0, margin_h + 1)
        offset_w = np.random.randint(0, margin_w + 1)
        crop_y1, crop_y2 = offset_h, offset_h + crop_size[0]
        crop_x1, crop_x2 = offset_w, offset_w + crop_size[1]
        ...
        # crop the image
        img_prev = img_prev[crop_y1:crop_y2, crop_x1:crop_x2, ...]
        img_now = img_now[crop_y1:crop_y2, crop_x1:crop_x2, ...]
        img_next = img_next[crop_y1:crop_y2, crop_x1:crop_x2, ...]
        
        img_shape = img_now.shape
    ...
    def __call__(self, results):
        ...
        Returns:
            dict: Randomly cropped results, 'img_shape' key in result dict is
                updated according to crop size.
        """
        image_size = results['img_now'].shape[:2]
        crop_size = self._get_crop_size(image_size)
        results = self._crop_data(results, crop_size, self.allow_negative_crop)
        return results
...
# 适用单样本3帧数据增强，数据集通用
@PIPELINES.register_module()
class MulPhotoMetricDistortionForFrames3:
    """Apply photometric distortion to image sequentially, every transformation
    is applied with a probability of 0.5. The position of random contrast is in
    second or second to last.
    ...
    def __call__(self, results):
        ...
        # 单样本当前帧
        img_now = results['img_now']
        img_now = img_now.astype(np.float32)
        # 单样本后帧
        img_next = results['img_next']
        img_next = img_next.astype(np.float32)
        
        # random brightness  随机亮度，确保单样本每帧使用相同的亮度
        if random.randint(2):
            delta = random.uniform(-self.brightness_delta,
                                   self.brightness_delta)
            img_prev += delta
        ...
        # convert color from BGR to HSV
        img_prev = mmcv.bgr2hsv(img_prev)
        img_now = mmcv.bgr2hsv(img_now)
        img_next = mmcv.bgr2hsv(img_next)
        
        # random saturation
        if random.randint(2):
            rs = random.uniform(self.saturation_lower,self.saturation_upper)
            img_prev[..., 1] *= rs
            img_now[..., 1] *= rs
            img_next[..., 1] *= rs
        ...
        # random hue
        if random.randint(2):
            rh = random.uniform(-self.hue_delta, self.hue_delta)
            img_prev[..., 0] += rh
            img_now[..., 0] += rh
            img_next[..., 0] += rh
            
            img_prev[..., 0][img_prev[..., 0] > 360] -= 360
            img_now[..., 0][img_now[..., 0] > 360] -= 360
            img_next[..., 0][img_next[..., 0] > 360] -= 360
            
            img_prev[..., 0][img_prev[..., 0] < 0] += 360
            img_now[..., 0][img_now[..., 0] < 0] += 360
            img_next[..., 0][img_next[..., 0] < 0] += 360

        # convert color from HSV to BGR
        img_prev = mmcv.hsv2bgr(img_prev)
        img_now = mmcv.hsv2bgr(img_now)
        img_next = mmcv.hsv2bgr(img_next)
        ...
        # randomly swap channels
        if random.randint(2):
            rp = random.permutation(3)
            img_prev = img_prev[..., rp]
            img_now = img_now[..., rp]
            img_next = img_next[..., rp]
...
# 适用单样本5帧数据增强, 数据集通用
@PIPELINES.register_module()
class MulPhotoMetricDistortionForFrames5:
    """Apply photometric distortion to image sequentially, every transformation
    is applied with a probability of 0.5. The position of random contrast is in
    second or second to last.
    ...
    def __call__(self, results):
        ...
        """
        # 单样本前帧
        img_prev_prev = results['img_prev_prev']
        img_prev_prev = img_prev_prev.astype(np.float32)
        ...
        # 单样本后后帧
        img_next_next = results['img_next_next']
        img_next_next = img_next_next.astype(np.float32)
        ...
        # convert color from BGR to HSV
        img_prev_prev = mmcv.bgr2hsv(img_prev_prev)
        img_prev = mmcv.bgr2hsv(img_prev)
        img_now = mmcv.bgr2hsv(img_now)
        img_next = mmcv.bgr2hsv(img_next)
        img_next_next = mmcv.bgr2hsv(img_next_next)
        ...
        if random.randint(2):
            ...
            img_prev_prev[..., 0][img_prev_prev[..., 0] > 360] -= 360
            img_prev[..., 0][img_prev[..., 0] > 360] -= 360
            img_now[..., 0][img_now[..., 0] > 360] -= 360
            img_next[..., 0][img_next[..., 0] > 360] -= 360
            img_next_next[..., 0][img_next_next[..., 0] > 360] -= 360
            
            img_prev_prev[..., 0][img_prev_prev[..., 0] < 0] += 360
            img_prev[..., 0][img_prev[..., 0] < 0] += 360
            img_now[..., 0][img_now[..., 0] < 0] += 360
            img_next[..., 0][img_next[..., 0] < 0] += 360
            img_next_next[..., 0][img_next_next[..., 0] < 0] += 360

        # convert color from HSV to BGR
        img_prev_prev = mmcv.hsv2bgr(img_prev_prev)
        img_prev = mmcv.hsv2bgr(img_prev)
        img_now = mmcv.hsv2bgr(img_now)
        img_next = mmcv.hsv2bgr(img_next)
        img_next_next = mmcv.hsv2bgr(img_next_next)
        ...
        if random.randint(2):
            rp = random.permutation(3)
            img_prev_prev = img_prev_prev[..., rp]
            img_prev = img_prev[..., rp]
            img_now = img_now[..., rp]
            img_next = img_next[..., rp]
            img_next_next = img_next_next[..., rp]
