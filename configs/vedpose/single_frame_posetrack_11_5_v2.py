...
model = dict(
    ...
    init_cfg=dict(type='Pretrained', checkpoint='checkpoints/petr_r50_16x2_100e_coco_15keypoints.pth'),
    backbone=dict(
        type='mmdet.ResNet',
        depth=50,
        num_stages=4,
    ...
    bbox_head=dict(
        ...
        loss_cls=dict(
            ...
            alpha=0.25,
            loss_weight=4.0),
        # encoder:120, cls:4, oks:14, pose_rle:1, joint_rle:1
        loss_kpt=dict(type='opera.RLELoss', loss_weight=1.0, joints_weight=[1., 1.,1., 1., 1., 1.2, 1.2, 1.5, 1.5, 1., 1., 1.2, 1.2, 1.5, 1.5]),
        loss_kpt_rpn=dict(type='mmdet.L1Loss', loss_weight=140.0),
        loss_oks=dict(type='opera.OKSLoss', num_keypoints=15, loss_weight=4.0),
        ...
        loss_oks_refine=dict(type='opera.OKSLoss', num_keypoints=15, loss_weight=6.0)),
    train_cfg=dict(
        assigner=dict(
            type='opera.PoseHungarianAssigner',
            cls_cost=dict(type='mmdet.FocalLossCost', weight=4.0),
            kpt_cost=dict(type='opera.KptL1Cost', weight=140.0),
