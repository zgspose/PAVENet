...
model = dict(
    ...
    init_cfg=dict(type='Pretrained', checkpoint='work_dirs/2024_10_6_videopose_r50_20e_coco_fuse_time_v1/epoch_7.pth'),
    backbone=dict(
        type='mmdet.ResNet',
    ...
    neck=dict(
        ...
        num_outs=4),
    bbox_head=dict(
        type='opera.VideoPoseHeadV6',
        num_keypoints=15,
        num_query=300,
        num_classes=1,  # only person
        in_channels=2048,
        joint_space_time_decoder=True, # 在joint-decoder中使用时空解码
        sync_cls_avg_factor=True,
        with_kpt_refine=True,
        as_two_stage=True,
        transformer=dict(
            type='opera.VideoPoseTransformerV5',
            num_keypoints=15,
            encoder=dict(
                ...
                transformerlayers=dict(
                    ...
                    attn_cfgs=[
                        dict(
                            ...
                            dropout=0.1),
                        dict(
                            type='mmcv.MulFramesMultiScaleDeformableAttentionV1',
                            embed_dims=256,
                            im2col_step=128)
                    ],
                    feedforward_channels=1024,
