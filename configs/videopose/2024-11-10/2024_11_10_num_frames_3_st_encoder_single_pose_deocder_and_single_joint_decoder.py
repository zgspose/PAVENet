...
model = dict(
    ...
    neck=dict(
        ...
        num_outs=4),
    bbox_head=dict(
        type='opera.VideoPoseHeadV11_10_1',
        num_keypoints=15,
        num_query=300,
        ...
        as_two_stage=True,
        transformer=dict(
            type='opera.PETRTransformerV11_10_1',
            num_keypoints=15,
            encoder=dict(
                type='mmcv.STDetrTransformerEncoder',
                num_layers=6,
                transformerlayers=dict(
                    type='mmcv.ModifyBaseTransformerLayer',
                    attn_cfgs=[
                        dict(
                            type='mmcv.MultiScaleDeformableAttention',
                            embed_dims=256),
                        dict(
                            type='mmcv.TimeMultiheadAttention',
                            embed_dims=256,
                            num_heads=8,
                            dropout=0.1),
                        ],
                    ...
                    ffn_dropout=0.1,
                    operation_order=('self_attn', 'norm', 'time_attn', 'norm', 'ffn', 'norm'))),
            decoder=dict(
