...
# pose-decoder 时空解码 + joint-decoder 时空解码 ------ 三帧只有一份task_query,使用三个不同的线形层得到不同帧的相对偏移
model = dict(
    type='opera.VideoPoseV1',
    # 加载预训练权重  ----- 在posetracks上使用pose时空解码训练好的，76.7
    init_cfg=dict(type='Pretrained', checkpoint='work_dirs/2024_10_21_videopose_r50_20e_coco_fuse_time_v1/epoch_10.pth'),
    backbone=dict(
        type='mmdet.ResNet',
    ...
    bbox_head=dict(
        ...
        loss_kpt_rpn=dict(type='mmdet.L1Loss', loss_weight=180.0),
    ...
    train_cfg=dict(
        assigner=dict(
            ...
            kpt_cost=dict(type='opera.KptL1Cost', weight=180.0),
