...
# backbone + encoder 固定
# pose-decoder 固定 （时空解码  ------ 三帧只有一份task_query,使用三个不同的线形层得到不同帧的相对偏移以及回归不同帧的参考点位置偏移）
# joint-decoder 训练 （时空解码  ------ 三帧只有一份task_query,初始位置使用pose输出的当前帧结果，使用三个不同的线形层得到不同帧的相对偏移以及回归不同帧的参考点位置偏移）
model = dict(
    type='opera.VideoPoseV1',
    # 加载预训练权重  ----- 使用只训练backbone + encoder + pose-decoder(时空解码) + Joint-decoder(单帧)权重
    init_cfg=dict(type='Pretrained', checkpoint='work_dirs/2024_11_9_num_frames_3_pose_deocder_and_joint_decoder_use_now_frame_result/epoch_9.pth'),
    backbone=dict(
        type='mmdet.ResNet',
    ...
    neck=dict(
        ...
        num_outs=4),
    bbox_head=dict(
        type='opera.VideoPoseHeadV11_9_1_freeze',
        num_keypoints=15,
        num_query=300,
        ...
        as_two_stage=True,
        transformer=dict(
            type='opera.VideoPoseTransformerV11_9_1_freeze',
            num_keypoints=15,
            encoder=dict(
