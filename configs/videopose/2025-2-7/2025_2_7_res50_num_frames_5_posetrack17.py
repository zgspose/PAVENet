_base_ = [
    '../../_base_/datasets/posetrack17_video_keypoint_num_frame_5.py', '../../_base_/default_runtime.py'
]
# backbone + encoder 训练
# pose-decoder 训练 （时空解码  ------ 5帧只有一份task_query,使用5个不同的线形层得到不同帧的相对偏移以及回归不同帧的参考点位置偏移）
# joint-decoder 训练 （时空解码  ------ 5帧只有一份task_query,初始位置使用pose输出的当前帧结果，使用5个不同的线形层得到不同帧的相对偏移以及回归不同帧的参考点位置偏移）
# end-to-end
model = dict(
    type='opera.VideoPoseV1',
    # 加载预训练权重  ----- 使用coco模拟时序训练好的权重
    init_cfg=dict(type='Pretrained', checkpoint='pretrain_checkpoints/resnet50_coco_rle_frames_5_15keypoints.pth'),
    backbone=dict(
        type='mmdet.ResNet',
