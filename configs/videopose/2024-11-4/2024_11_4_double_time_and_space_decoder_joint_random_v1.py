_base_ = [
    '../../_base_/datasets/posetrack_video_keypoint_num_frame_5.py', '../../_base_/default_runtime.py'
]
# pose-decoder 时空解码 + joint-decoder 时空解码 ------ 五帧只有一份task_query,使用五个不同的线形层得到不同帧的相对偏移
model = dict(
    type='opera.VideoPoseV1',
    # 加载预训练权重  ----- 在posetracks上使用pose时空解码训练好的，76.7 work_dirs/2024_10_21_videopose_r50_20e_coco_fuse_time_v1/epoch_10.pth
    # 单帧训练好的  work_dirs/single_frame_posetrack_11_1_v1/epoch_24.pth
    init_cfg=dict(type='Pretrained', checkpoint='work_dirs/single_frame_posetrack_11_1_v1/epoch_24.pth'),
    backbone=dict(
        type='mmdet.ResNet',
    ...
    bbox_head=dict(
        ...
        as_two_stage=True,
        transformer=dict(
            type='opera.VideoPoseTransformerV17_2',
            num_keypoints=15,
            num_frames=5,
            encoder=dict(
            ...
            decoder=dict(
                ...
                transformerlayers=dict(
                    ...
                    attn_cfgs=[
                        ...
                        # 使用不同的线性层
                        dict(
                            type='opera.MulFramesMultiScaleDeformablePoseAttentionV5',
                            num_points=15,
                            embed_dims=256)
                    ],
                    feedforward_channels=1024,
            ...
            refine_decoder=dict(
                ...
                transformerlayers=dict(
                    ...
                    attn_cfgs=[
                        ...
                        # 使用不同的线性层
                        dict(
                            type='mmcv.MulFramesMultiScaleDeformableAttentionV17_v2',
                            embed_dims=256,
                            im2col_step=128)
                    ],
                    feedforward_channels=1024,
        ...
        loss_kpt_refine=dict(type='opera.RLELoss', loss_weight=1.5, joints_weight=[1., 1.,1., 1., 1., 1.2, 1.2, 1.5, 1.5, 1., 1., 1.2, 1.2, 1.5, 1.5]),
    ...
    train_cfg=dict(
        assigner=dict(
            ...
            oks_cost=dict(type='opera.OksCost', num_keypoints=15, weight=14.0))),
    test_cfg=dict(max_per_img=50))  # set 'max_per_img=20' for time counting
# optimizer
optimizer = dict(
    type='AdamW',
    lr=5e-5,
