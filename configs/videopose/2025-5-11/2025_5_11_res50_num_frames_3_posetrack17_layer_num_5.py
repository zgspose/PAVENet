...
# backbone + encoder 训练
# pose-decoder 训练 （时空解码  ------ 3帧只有一份task_query,使用3个不同的线形层得到不同帧的相对偏移以及回归不同帧的参考点位置偏移）
# joint-decoder 训练 （时空解码  ------ 3帧只有一份task_query,初始位置使用pose输出的当前帧结果，使用3个不同的线形层得到不同帧的相对偏移以及回归不同帧的参考点位置偏移）
# end-to-end
model = dict(
    type='opera.VideoPoseV1',
    # 加载预训练权重  ----- 使用coco模拟时序训练好的权重
    init_cfg=dict(type='Pretrained', checkpoint='pretrain_checkpoints/resnet50_coco_rle_frames3_15keypoints_with_num_layer_5.pth'),
    backbone=dict(
        type='mmdet.ResNet',
