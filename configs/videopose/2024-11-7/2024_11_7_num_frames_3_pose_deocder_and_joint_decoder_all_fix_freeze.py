...
model = dict(
    ...
    # 加载预训练权重  ----- 在posetracks单帧训练最好的权重
    init_cfg=dict(type='Pretrained', checkpoint='work_dirs/2024_11_7_num_frames_3_only_pose_decoder_v1/epoch_19.pth'),
    backbone=dict(
        type='mmdet.ResNet',
    ...
    neck=dict(
        ...
        num_outs=4),
    bbox_head=dict(
        type='opera.VideoPoseHeadV17_5_freeze',
        num_keypoints=15,
        num_query=300,
        ...
        as_two_stage=True,
        transformer=dict(
            type='opera.VideoPoseTransformerV17_5_freeze',
            num_keypoints=15,
            encoder=dict(
                ...
                transformerlayers=dict(
                    ...
                    attn_cfgs=[
                        dict(
                            ...
                            num_heads=8,
                            dropout=0.1),
                        # 修改 --- 使用不同的线性层回归不同的偏移量用于查找feature-token,同时辅助帧也回归相对偏移
                        dict(
                ...
                transformerlayers=dict(
                    ...
                    attn_cfgs=[
                        dict(
                            ...
                            num_heads=8,
                            dropout=0.1),
                        # 使用不同的线性层回归不同的偏移量用于查找feature-token
                        dict(
        ...
        loss_cls=dict(
            ...
            alpha=0.25,
            loss_weight=0.0),
        loss_kpt=dict(type='opera.RLELoss', loss_weight=0.0, joints_weight=[1., 1., 1., 1., 1., 1.2, 1.2, 1.5, 1.5, 1., 1., 1.2, 1.2, 1.5, 1.5]),
        loss_kpt_rpn=dict(type='mmdet.L1Loss', loss_weight=0.0),
