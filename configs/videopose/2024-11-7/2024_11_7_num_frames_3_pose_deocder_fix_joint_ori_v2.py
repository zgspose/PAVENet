...
model = dict(
    ...
    init_cfg=dict(type='Pretrained', checkpoint='work_dirs/single_frame_posetrack_11_1_v1/epoch_22.pth'),
    backbone=dict(
        type='mmdet.ResNet',
    ...
    bbox_head=dict(
        ...
        as_two_stage=True,
        transformer=dict(
            type='opera.VideoPoseTransformerV17_5',
            num_keypoints=15,
            encoder=dict(
                ...
                transformerlayers=dict(
                    ...
                    attn_cfgs=[
                        dict(
                            ...
                            num_heads=8,
                            dropout=0.1),
                        # 使用不同的线性层回归不同的偏移量用于查找feature-token,同时辅助帧也回归相对偏移
                        dict(
